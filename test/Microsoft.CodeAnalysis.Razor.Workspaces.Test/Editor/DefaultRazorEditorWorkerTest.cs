...
{
    public class DefaultRazorEditorWorkerTest
    {
        [Fact]
        public async Task RazorEditorWork_ParseAsync_ParsesInitialDocument()
        {
            // Arrange
            var workspace = new AdhocWorkspace();
            var project = workspace.AddProject("WebApplication1", LanguageNames.CSharp);

            var cSharp = project.AddDocument("Test.cshtml", SourceText.From(""));
            var razor = cSharp.Project.AddAdditionalDocument("Test.cshtml", SourceText.From("<p>Hello, World!</p>"));
            var container = new TestSourceTextContainer(workspace, razor.Id);

            Assert.True(workspace.TryApplyChanges(razor.Project.Solution));

            var templateEngine = CreateTemplateEngine();

            var parser = new DefaultRazorEditorWorker(new DefaultThreadDispatcher(), workspace, templateEngine, razor.FilePath, cSharp.Id, container);
        ...
        private RazorTemplateEngine CreateTemplateEngine()
        {
        ...
        public static TheoryData TagHelperPartialParseRejectData
        {
            get
            {
                ...
                return new TheoryData<SourceChange, MarkupBlock>
                {
                    {
        ...
        public void TagHelperTagBodiesRejectPartialChanges(TextChange change, object expectedDocument)
        {
            ...
            var parser = new DefaultRazorEditorWorker()
        ...
        private static SourceChange CreateInsertionChange(string initialText, int position, string insertionText)
        {
            var change = new SourceChange(position, 0, )

            var changedText = initialText.Insert(position, insertionText);

            var original = new StringTextBuffer(initialText);
            var changed = new StringTextBuffer(changedText);
            return new TextChange(position, 0, original, insertionText.Length, changed);
        }
        ...
        public class DocumentAndChange
        {
            public DocumentAndChange()
            {
                new SourceTextContainer.
            }

            public SourceText Before { get; }
        }
